...
if  [ "$FRONTENDRUNNING" == "false" ]; then
    ...
    docker run -d -p 8080:8080 -p 80:80 -p 443:443 --name=frontend  --restart=always -v /var/run/docker.sock:/var/run/docker.sock harbor.pbndev.net:3443/traefik:latest
...
POSTDOCKERTOTAL=$(($POSTDOCKEREND - $POSTDOCKERSTART));
